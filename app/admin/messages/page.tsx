...
function getMessageStatusSelectClass(status: string) {
  return `h-9 w-32 rounded-full px-3 text-xs font-black uppercase tracking-wide shadow-none ${getMessageStatusStyle(status)}`;
}
            ...
            <div className="grid gap-3">
              <div className="flex items-start justify-between gap-3">
                ...
                <Select
                  ...
                  className={getMessageStatusSelectClass(message.status)}
                />
              </div>
              <div className="flex flex-wrap items-center gap-3 text-sm text-slate-500">
                ...
                {message.email ? <a href={`mailto:${message.email}`} className="break-words hover:underline">{message.email}</a> : null}
              </div>
              <p className="rounded-md bg-slate-50 px-3 py-2 text-sm text-slate-700">{message.message}</p>
